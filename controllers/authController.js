...
const checkEnrollment = async (req, res) => {
  try {
    ...
    if (existingPayment !== undefined) {
      ...
      return res.status(200).send({
        success: false,
        message: "User already enrolled for this month",
      });
...
const changeBatch = async (req, res) => {
  try {
    ...
    // checking if the user has an enrollment in the current month
    const [existingPayment] = await queryAsync(
      'SELECT users.Name AS userName, DATE_FORMAT(DateOfPayment, "%Y-%m") AS paymentMonth, payment.BatchID as batchid ' +
      'FROM payment ' +
      'INNER JOIN users ON payment.UserID = users.UserID ' +
    ...
    if (existingPayment !== undefined) {
      ...
      return res.status(400).json({ success: false, message: 'You are not Enrolled.subscribe to Enroll' });
    ...
    // Check if the user is already enrolled in the selected batch

    const [currentUserEnrollment] = await queryAsync('SELECT BatchID FROM payment WHERE UserID = ?', [userId]);
    if (currentUserEnrollment && currentUserEnrollment.BatchID == batchId) {
      console.log("validating current enrolmment");
      return res.status(200).json({ success: true, message: 'User is already enrolled in the selected batch', enrolledBatchId: batchId });
    }
...
//to delete enrollment for current month
const deleteEnrollment = async (req, res) => {
  try {
    ...
    // If the user has an existing payment for the current month, return the enrollment status
    if (existingPayment !== undefined) {
      await queryAsync('DELETE FROM payment WHERE UserID = ? AND DATE_FORMAT(CURDATE(), "%Y-%m") = DATE_FORMAT(DateOfPayment, "%Y-%m")', [userId]);
      return res.status(200).json({ success: true, message: 'Enrollment deleted for the current month as user is enrolled' });
     
      
    } else {
      // User is not enrolled for the current month, delete the enrollment
      return res.status(200).json({ success: true, message: 'User is not enrolled for the current month'});
    }
  } catch (error) {
